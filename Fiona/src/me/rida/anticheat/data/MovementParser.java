...
import me.rida.anticheat.utils.blockbox.CollisionAssessment;
...
public class MovementParser {
    ...
    public static boolean parse(WrappedInFlyingPacket packet, PlayerData data) {
        ...
        data.addPastLocation(new AntiCheatLocation(to));
        ...
        List<BoundingBox> box = AntiCheat.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(to.getWorld(), data.boundingBox.grow(0.5f, 0.1f, 0.5f).subtract(0, 0.5f, 0, 0, 0, 0));
        ...
        AntiCheat.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(to.getWorld(), data.boundingBox.subtract(0, 2, 0, 0, 0, 0)).forEach(bb -> assessment.addMaterial(BlockUtils.getBlock(bb.getMinimum().toLocation(data.player.getWorld()))));
        ...
        data.riptideTicks = AntiCheat.getInstance().getBlockBoxManager().getBlockBox().isRiptiding(data.player) ? Math.min(40, data.riptideTicks + 1) : Math.max(0, data.riptideTicks - 1);
        ...
        if (packet.isLook() || packet.isPos()) {
            PacketFunkeMoveEvent event = new PacketFunkeMoveEvent(data.player, new AntiCheatLocation(data.movement.from), new AntiCheatLocation(to), packet.isGround(), data.movement.hasJumped);
