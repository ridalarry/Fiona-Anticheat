...
import me.rida.anticheat.utils.math.*;
...
public class PlayerData {
    ...
    public AntiCheatLocation lastTo;
    ...
    public List<AntiCheatLocation> pastLocation = Lists.newArrayList();
    ...
    public float getViolations(Check check) {
        if (AntiCheat.getInstance().getCheckManager().violations.containsKey(player.getUniqueId())) {
            List<Violation> violationList = AntiCheat.getInstance().getCheckManager().violations.get(player.getUniqueId());
    ...
    public AntiCheatLocation getLocationByTime(long millis) {
    ...
    public void addPastLocation(AntiCheatLocation anticheatLocation) {
        if (pastLocation.size() >= 20) {
            pastLocation.remove(0);
        }

        pastLocation.add(anticheatLocation);
    }
