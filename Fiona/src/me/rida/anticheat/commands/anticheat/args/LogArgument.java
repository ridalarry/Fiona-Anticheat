...
import me.rida.anticheat.data.logging.Log;
...
public class LogArgument extends FunkeArgument {
    ...
    public LogArgument() {
        super("log", "log <args> <player> [page]", "View the logs of a player.", "anticheat.log");
    ...
    public void onArgument(CommandSender sender, Command cmd, String[] args) {
        if (args.length > 2) {
            ...
            List<Log> logs = AntiCheat.getInstance().getDataManager().getLogger().getLogs(player.getUniqueId());
            ...
            if (logs.size() > 0) {
                switch (args[1].toLowerCase()) {
                    ...
                    case "vl":
                        ...
                        logs.forEach(log -> {
                            Violation violation = violations.stream().filter(vl -> vl.getCheck().getName().equalsIgnoreCase(log.getCheck())).findFirst().orElse(new Violation(AntiCheat.getInstance().getCheckManager().getCheckByName(log.getCheck())));
                    ...
                    case "clear":
                    case "delete": {
                        Yaml yaml = (Yaml) AntiCheat.getInstance().getDataManager().getLogger();
                ...
                sender.sendMessage(MiscUtils.formattedMessage(Color.Red + "AntiCheat has no logs stored for this player."));
            }
            sender.sendMessage(MiscUtils.line(Color.Dark_Gray));
        } else {
