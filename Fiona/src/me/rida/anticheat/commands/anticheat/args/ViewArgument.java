...
public class ViewArgument
    ...
    public ViewArgument() {
        super("view", "view <player>", "view a player's data collected by AntiCheat.", "anticheat.violations");
    }
    ...
    public void onArgument(CommandSender sender, Command command, String[] args) {
        if (args.length == 2) {
            ...
            if (data == null) {
                sender.sendMessage(Color.Red + "Unknown error occurred that prevented AntiCheat from accessing the target's data.");
                return;
            }
            ...
            if (!AntiCheat.getInstance().getCheckManager().violations.containsKey(data.player.getUniqueId())) {
                sender.sendMessage(Color.Gray + "None! :)");
            } else {
                AntiCheat.getInstance().getCheckManager().violations.get(data.player.getUniqueId()).forEach(vl -> {
