...
public abstract class FunkeCommand
    ...
    protected FunkeCommand(String name, String display, String description, String permission) {
        ...
        instance = this;
        AntiCheat.getInstance().getCommand(name).setExecutor(this);
        AntiCheat.getInstance().getCommand(name).setTabCompleter(this);
        this.addArguments();
    }
                    ...
                    if (conditional.length > 0) {
                        if (args[Integer.parseInt(conditional[1]) - 1].equalsIgnoreCase(conditional[0].replaceAll("!", "")) == !conditional[0].startsWith("!")) {
                            switch (arg.toLowerCase()) {
                                case "%check%":
                                    AntiCheat.getInstance().getCheckManager().getChecks().stream().filter(check -> check.getName().toLowerCase().replaceAll(" ", "_").startsWith(args[args.length - 1].toLowerCase())).forEach(check -> toReturn.add(check.getName().toLowerCase().replaceAll(" ", "_")));
                                    break;
                                case "%detection%":
                                    Check check = AntiCheat.getInstance().getCheckManager().getCheckByName(args[args.length - 2]);
                                    assert check != null;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (this.permission != null && !sender.hasPermission(this.permission)) {
            sender.sendMessage(getName().equalsIgnoreCase("anticheat") ? Color.Red + "This server is using AntiCheat AntiCheat v" + AntiCheat.getInstance().getDescription().getVersion() + " by funkemunky, XtasyCode, and Martin." : AntiCheat.getInstance().getMessageFields().noPermission);
            return true;
        }
        try {
            ...
            for (FunkeArgument argument : this.arguments) {
                ...
                if ((argument.getPermission() == null || sender.hasPermission("anticheat.admin")
                        || sender.hasPermission(permission))) {
                    argument.onArgument(sender, cmd, args);
                    break;
                }
                sender.sendMessage(AntiCheat.getInstance().getMessageFields().noPermission);
                break;
            }
        }
        return true;
