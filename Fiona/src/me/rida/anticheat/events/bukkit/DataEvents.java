...
import me.rida.anticheat.data.MovementParser;
...
import me.rida.anticheat.events.custom.*;
import me.rida.anticheat.events.system.EventManager;
import me.rida.anticheat.events.system.EventMethod;
import me.rida.anticheat.events.system.Listener;
...
public class DataEvents
    ...
    public void onPacketReceive(PacketRecieveEvent e) {
        ...
        switch (e.getType()) {
            ...
            case Packet.Client.LEGACY_POSITION_LOOK: {
                ...
                if ((data.isUsingItem = AntiCheat.getInstance().getBlockBoxManager().getBlockBox().isUsingItem(e.getPlayer()))) {
    ...
    public void onInventoryOpen(InventoryOpenEvent event) {
        if (event.getPlayer() instanceof Player) {
            PlayerData data = AntiCheat.getInstance().getDataManager().getPlayerData((Player) event.getPlayer());
    ...
    public void onPacketAttack(PacketAttackEvent event) {
        PlayerData data = AntiCheat.getInstance().getDataManager().getPlayerData(event.getAttacker());
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        if (event.getDamager() instanceof Player) {
            PlayerData data = AntiCheat.getInstance().getDataManager().getPlayerData((Player) event.getDamager());
        ...
        if (data != null) {
            if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
                if (event.getItem() != null
                    ...
                    for (BoundingBox box : AntiCheat.getInstance().getBlockBoxManager().getBlockBox().getSpecificBox(block.getLocation())) {
    ...
    public void onEntityDmaage(EntityDamageEvent event) {
        if (event.getEntity() instanceof Player) {
            PlayerData data = AntiCheat.getInstance().getDataManager().getPlayerData((Player) event.getEntity());
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        AntiCheat.getInstance().profile.start("data:BlockPlaceEvent");
        if (event.isCancelled()) {
        ...
        AntiCheat.getInstance().profile.stop("data:BlockPlaceEvent");
    }

    @EventHandler
    public void onProjectile(ProjectileLaunchEvent event) {
        if (event.getEntity().getShooter() instanceof Player) {
            PlayerData data = AntiCheat.getInstance().getDataManager().getPlayerData((Player) event.getEntity().getShooter());
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        AntiCheat.getInstance().profile.start("data:BlockBreakEvent");
        PlayerData data = AntiCheat.getInstance().getDataManager().getPlayerData(event.getPlayer());
        if (data != null) {
            callBukkitEvent(event, data);
            if (data.cancelType == CheckType.WORLD && data.cancelTicks > 0) {
        ...
        AntiCheat.getInstance().profile.stop("data:BlockBreakEvent");
    }
    ...
    private void callBukkitEvent(Event event, PlayerData data) {
        try {
            AntiCheat.getInstance().executorTwo.execute(() -> {
                if (data.boundingBox != null) {
                    AntiCheat.getInstance().getCheckManager().getChecks().stream().filter(Check::isEnabled).forEach(check -> check.getDetections().stream().filter(Detection::isEnabled).forEach(detection -> {
                        try {
                            AntiCheat.getInstance().profile.start("check:" + check.getName() + ":" + detection.getId());
                            detection.onBukkitEvent(event, data);
                            AntiCheat.getInstance().profile.stop("check:" + check.getName() + ":" + detection.getId());
                        } catch (Exception e) {
                            new BukkitRunnable() {
                                public void run() {
                                    try {
    ...
    private void callFunkeEvent(me.rida.anticheat.events.system.Event event, PlayerData data) {
        try {
            if (data.boundingBox != null) {
                AntiCheat.getInstance().executorOne.execute(() -> AntiCheat.getInstance().getCheckManager().getChecks().stream().filter(Check::isEnabled).forEach(check -> {
