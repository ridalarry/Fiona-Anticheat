        ...
        if (event instanceof PacketFunkeMoveEvent) {
            ...
            if (MathUtils.playerMoved(e.getFrom(), e.getTo())) {
                ...
                if (!PlayerUtils.isRiskyForFlight(data)
                        ...
                        && !data.onGround
                        && AntiCheat.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(data.player.getWorld(),  data.boundingBox.subtract(0, (float) (double) getConfigValues().get("expandBoxBy"), 0, 0, 0, 0)).size() == 0) {
